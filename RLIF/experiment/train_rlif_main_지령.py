...
FLAGS_DEF = define_flags_with_default(
    project_name="quality_intervene", # rlif_pen_v2
    #project_name="test_pen",
    env_name="hopper-medium-replay-v2", # hopper-medium-v2
...
def main(_):
    ...
    for i in tqdm.tqdm(
        ...
        if not intervene:
            ...
            time_factor = min(i / FLAGS.max_steps, 1) # time_factor: 0 ~ 1 사이 값, 시간이 지날수록 감소

            if policy_q < gt_q * (FLAGS.intervene_threshold * time_factor):
                intervene = np.random.choice([0, 1], p=[0.05, 1-0.05]) # default: 개입하지 않을 확률 5%, 개입할 확률 95% 
            else:
                intervene = np.random.choice([0, 1], p=[1-0.05, 0.05]) # default: 개입하지 않을 확률 95%, 개입할 확률 5% 
            '''
            '''
            # Intervene 2
            if policy_q < gt_q * FLAGS.intervene_threshold:
                intervene = np.random.choice([0, 1], p=[0.03, 1-0.03]) 
            else:
                intervene = np.random.choice([0, 1], p=[1-0.03, 0.03]) # default: 개입하지 않을 확률 97%, 개입할 확률 3% 
            '''
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            def run_evaluation_step(i, FLAGS, model_dir, eval_env, all_intervene, sparse_eval_sampler):
                print(f"============{i}============")
                video_folder = f"results_video_110EA-MR_{i}"
                ...
                for j in range(20):
                    ...
                    save_to_csv('evaluation_log_110EA-MR.csv', log_data)
