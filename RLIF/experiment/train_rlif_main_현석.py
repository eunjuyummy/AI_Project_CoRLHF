...
#from RLIF.utils.env_utils import GymnasiumWrapper, TrajSampler, get_d4rl_dataset, evaluate, wrap_gym
...
FLAGS_DEF = define_flags_with_default(
    project_name="hs_quality_intervene_2", # rlif_pen_v2
    #project_name="test_pen",
    env_name="hopper-medium-v2", # hopper-medium-v2 # "hopper-expert-v2"
    sparse_env='Hopper-v4', # AdroitHandPenSparse-v1 Hopper-v2
    offline_ratio=0.5,
    seed=43,  
    train_sparse=True, # mod
    dataset_dir='',
    # ./RLIF/experts/rlpd_experts/s24_hopper-expert-v2env/model.pkl
    # ./RLIF/experts/bc_experts/960d7aba1a654a6aae323b7558bf3378/model.pkl
    ...
    # bc_experts = 40, expert dir rlpd_experts = 110
...
def main(_):
    ...
    for i in tqdm.tqdm(
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            # 역량 기반 개입 방식 
            #recent_returns = []
            #recent_return = eval_info.get('return', 0)
            #recent_returns.append(recent_return)
            # 최근 10회의 평가 결과만 유지하도록 설정
            #if len(recent_returns) > 10:
                # recent_returns.pop(0)
            # 최근 10회의 평가 결과를 기준으로 성능 평가
    ...
    for i in tqdm.tqdm(
        ...
        if not intervene:
            ...
            if policy_q < gt_q * FLAGS.intervene_threshold:
                ...
                intervene = np.random.choice([0, 1], p=[1-0.05, 0.05]) # 개입하지 않을 확률 95%, 개입할 확률 5%
        ...
        # else:
           # action = policy_action
            # action = 0.7 * policy_action + 0.3 * expert_action
        # else:
           #  action = policy_action

            # 점진적 개입 방식 (EAM)
            ratio = max(0.7 - (i / FLAGS.max_steps) * 0.4, 0.3)
            action = ratio * expert_action + (1 - ratio) * policy_action
        else:
            action = policy_action

            # 점진적 개입 방식 (EAE)
            #ratio = max(1 - i / FLAGS.max_steps, 0.6)
            #expert_ratio = 0.2 + (0.4 - 0.2) * (1 - ratio)
            #action = (1 - expert_ratio) * policy_action + expert_ratio * expert_action
        #else:
            #action = policy_action  
            

            # 역량 기반 개입 방식 
            # if average_return < 2000:
               # action = 0.8 * policy_action + 0.2 * expert_action
            # else:
               # action = 0.5 * policy_action + 0.5 * expert_action
        # else:
            # action = policy_action
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            def run_evaluation_step(i, FLAGS, model_dir, eval_env, all_intervene, sparse_eval_sampler):
                print(f"============{i}============")
                video_folder = f"results_video_110EAM_v1_{i}" 

                vec_env = gymnasium.make('Hopper-v4', render_mode="rgb_array", reset_noise_scale=0.005 * i)  # 가변 reset_noise_scale
                ...
                for j in range(20):
                    ...
                    save_to_csv('evaluation_log_110EAM_v1.csv', log_data)
